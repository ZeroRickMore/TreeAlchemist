...
class Info:
    ...
    def set_wrc_info_type(self, type_ : str):
        self.type = type_
        if self.validate_wrc_info_type():
            ...
            ExitUtils.exit_with_error(f'You cannot set <info type=""> of node {self.relative_node_name} to {type_} of type {type(type_)}. {Info.get_wrc_info_type_allow_criteria()}')
    ...
    def set_wrc_info_info(self, info : str):
        self.info = info
        if self.validate_wrc_info_info():
            if self.print_diagnostics:
            ...
            ExitUtils.exit_with_error(f"You cannot set <info> of node {self.relative_node_name} to {info} of type {type(info)}. {Info.get_wrc_info_info_allow_criteria()}")
