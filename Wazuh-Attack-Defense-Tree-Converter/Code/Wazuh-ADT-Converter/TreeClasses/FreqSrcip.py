...
class FreqSrcip:
    ...
    def set_wrc_srcip_negate(self, negate : str):
        ...
        if self.validate_wrc_srcip_negate():
            ...
            ExitUtils.exit_with_error(f'You cannot set <srcip negate=""> of node {self.relative_node_name} to {negate} of type {type(negate)}. {FreqSrcip.get_wrc_srcip_negate_allow_criteria()}')
    ...
    def set_wrc_srcip_srcip(self, srcip : str):
        ...
        if self.validate_wrc_srcip_srcip():
            ...
            ExitUtils.exit_with_error(f'You cannot set <freq_srcip> of node {self.relative_node_name} to {srcip} of type {type(srcip)}. {FreqSrcip.get_wrc_srcip_srcip_allow_criteria()}')
    ...
    def validate_all(self):
        ...
        if not self.validate_wrc_srcip_negate():
            ExitUtils.exit_with_error(f'{error_prefix} <srcip negate="{self.get_wrc_srcip_negate()}"> in <wazuh_rule_config>. {FreqSrcip.get_wrc_srcip_negate_allow_criteria()} {self.srcip} of type {type(self.srcip)} {error_suffix}')
        if not self.validate_wrc_srcip_srcip():
            ExitUtils.exit_with_error(f'{error_prefix} <srcip> in <wazuh_rule_config>. {FreqSrcip.get_wrc_srcip_srcip_allow_criteria()} {self.get_wrc_srcip_srcip()} of type {type(self.get_wrc_srcip_srcip())} {error_suffix}')
