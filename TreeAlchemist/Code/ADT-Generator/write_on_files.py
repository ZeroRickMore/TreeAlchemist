...
class FilesWriter:
    ...
    def generate_guide_file_string(self):
        ...
        daemon_trees_directory_path = self.daemon_file.replace(os.path.join('ADT-Generator', 'Output-Files', self.root_name, f'{self.root_name}_daemon_readable.txt'), os.path.join('Daemons', 'Trees'))
        string += f'\n\nDaemon readable txt=======================\n\n\t- Go into the directory where the daemon reads file from.\n\tIt is the "Trees" directory located where wazuh_adtmanagerd.py is located.\n\tIf you did not touch the github code, it is inside /TreeAlchemist/Code/Daemons/ .\n\t- Move \n\t\t[ {self.daemon_file} ]\n\t inside of it.\n\nCommands for untouched github folders:\n\n\tcp {self.daemon_file} {daemon_trees_directory_path}'
